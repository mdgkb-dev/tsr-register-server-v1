...
func (s *Service) CreateMany(items []*models.Disability) error {
	...
	err := period.CreateService(s.helper).CreateMany(models.GetPeriodsFromDisability(items))
	if err != nil {
		return err
	}
	...
	err = edv.CreateService(s.helper).CreateMany(models.GetEdvs(items))
	if err != nil {
		return err
	}
	return nil
...
func (s *Service) UpsertMany(items []*models.Disability) error {
	...
	err := period.CreateService(s.helper).UpsertMany(models.GetPeriodsFromDisability(items))
	if err != nil {
		return err
	}
	models.SetPeriodIDToDisabilities(items)
	...
	edvService := edv.CreateService(s.helper)
